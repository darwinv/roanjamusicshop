...
$(document).ready(function(){
	if(checkIfIncluded("soundmanager2.js")){
		...
		loadjscssfile("{$modules_dir}prestamusicshop/css/mp3-player-button.css", "css") //dynamically load and add this .css file
		...
		loadjscssfile("{$modules_dir}prestamusicshop/css/mp3-player-button.css", "css")
...
{foreach from=$associated_mp3 item=items*}
    <p class="music_shop_item">
    	<a data-href="{$modules_dir}prestamusicshop/mp3/{$associated_mp3.mp3_name}" title="{$associated_mp3.author}-{$associated_mp3.mp3_title}" class="sm2_button">{$associated_mp3.author}-{$associated_mp3.mp3_title}</a>
        <span class="music_title">{$assoc_increment}. {$associated_mp3.author|truncate:15}-{$associated_mp3.mp3_title|truncate:15}</span>     
