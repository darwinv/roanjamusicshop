...
<script src='{$modules_dir}roanjamusicshop/js/soundmanager2en.js' type='text/javascript'></script>
<script>
    ...
    $(document).ready(function(){
    	if(checkIfIncluded("soundmanager2.js")){
    		...
    		loadjscssfile("{$modules_dir}roanjamusicshop/css/mp3-player-button15.css", "css") //dynamically load and add this .css file
    		loadjscssfile("{$modules_dir}roanjamusicshop/js/mp3-player-button.js", "js") //dynamically load and add this .js file
    	}else{
    		//loadjscssfile("{$modules_dir}roanjamusicshop/js/soundmanager2.js", "js")
    		loadjscssfile("{$modules_dir}roanjamusicshop/css/mp3-player-button15.css", "css")
    		loadjscssfile("{$modules_dir}roanjamusicshop/js/mp3-player-button.js", "js")
    	}
    })
</script>
...
{foreach from=$associated_mp3 item=items}
    <p class="music_shop_item">
        <span>Colum15.tpl</span>
    	<a data-href="{$modules_dir}roanjamusicshop/mp3/{$items.mp3_name}" title="{$items.author}-{$items.mp3_title}" class="sm2_button">{$items.author}-{$items.mp3_title}</a>
...
{foreach from=$product_mp3 item=items}
    <p class="music_shop_item">
    	<a href="{$modules_dir}roanjamusicshop/mp3/{$items.mp3_name}" title="{$items.author}-{$items.mp3_title}" class="sm2_button">{$items.author}-{$items.mp3_title}</a>
        <span class="music_title">{$assoc_increment}. {$items.author|truncate:10}-{$items.mp3_title|truncate:10}</span>
